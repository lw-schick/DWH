...
--    to call long running SQL command inside a trigger or another stored
--    procedure (for example, large web cache deleting after data update).
--
--    I created the stored procedure using job object of SQL server agent.
--    You can run the sql command asynchronously directly from SQL, from
--    another stored procedure, task or another sql server command or from
--    external ASP/ASP Net application.
-- @example:
...
BEGIN
    ...
    execute msdb.dbo.sp_add_job @jobname, @owner_login_name=@owner, @job_id=@id OUTPUT

    --Specify a job server for the job
    execute msdb.dbo.sp_add_jobserver @job_id=@id

    --Specify a first step of the job - the SQL command
    --(@on_success_action = 3 ... Go to next step)
    execute msdb.dbo.sp_add_jobstep @job_id=@id, @step_name='Step1', @command = @sql,
    ...
    execute msdb.dbo.sp_add_jobstep @job_id=@id, @step_name='Step2', @command = @deletecommand

    --Start the job
    execute msdb.dbo.sp_start_job @job_id=@id
